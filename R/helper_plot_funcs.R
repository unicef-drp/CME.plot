...
#' @importFrom CME.assistant roundoff
...
check.cqt.vs.results.csv <- function(
  ...
  dt2 <- dt_long[dt_cqt]
  dt2[, diff := CME.assistant::roundoff(Value, 1) - CME.assistant::roundoff(csv_value,1)]
  if(mean(dt2$diff)==0) {
...
#' Show locally saved png in markdown
#' @param p0 use to get runname
#' @param subfolder sometimes it is "Sex-specific"
show.saved.plot <- function(p0 = p, subfolder = ""){
  fig.dir <- here::here("Plots", "PNG", subfolder, p0$runname)
  files <- list.files(fig.dir, full.names = TRUE)
  if(length(files)>0){ # if png files exist
    fig_to_show <- if (grepl(show_iso, files)) grep(show_iso, files, value = TRUE) else files[1]
    message("fig_to_show is ", fig_to_show)
    knitr::include_graphics(fig_to_show)
  }
}
...
source.R.files <- function(print_file_names = FALSE, dir0 = NULL){
  if(is.null(dir0)) {
