...
If this repo is private, consider download the repo and compile the library locally.
...
The script "R/1.helper_plot_funcs.R" contains some helper functions used in the main function `savePlotResults`. There are also some helpful functions to transform among different format of results: `get.cqt.from.results`, `check.cqt.vs.results.csv`

The script "R/2.mark_new_series.R" contains internal functions to highlight new entries used in the main function `savePlotResults`. `get.new.series` and `mark.new.series` are the main functions (not exported). The exported functions include `review.date.of.dataentry`, `copy_code_to_dropbox` (copy code to local directories), `copy_data_to_dropbox`

The tricky part is to determine which countries have updated VR by comparing values of all VR series from a new data file against an old one. So which files are used? You can check this using the exported `find.dir.for.VR.comparison` function, which by default picks the latest U5MR database from `this.year()`'s folder versus `last.year()`. To overwrite, supply `dir_new_data_U5MR` and `dir_old_data_U5MR` in the global environment and these two objects will be used for comparison. 
